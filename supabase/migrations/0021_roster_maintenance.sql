-- 0021_roster_maintenance.sql — safe pre-draw team maintenance.
--
-- Owners may correct names/nationalities or remove a team while the tournament
-- is still in setup. Both functions lock the tournament row, so they cannot
-- race the atomic draw and mutate a roster after fixtures have gone live.

create or replace function public.update_setup_team(
  p_tournament_id uuid,
  p_team_id uuid,
  p_team_name text,
  p_players jsonb
)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_status public.tournament_status;
  v_team_size integer;
  v_player_count integer;
  v_updated integer;
begin
  select status, team_size
    into v_status, v_team_size
    from public.tournament
   where id = p_tournament_id
   for update;

  if not found then
    raise exception 'tournament_not_found';
  end if;
  if v_status <> 'setup' then
    raise exception 'roster_locked';
  end if;
  if not exists (
    select 1
      from public.team
     where id = p_team_id
       and tournament_id = p_tournament_id
  ) then
    raise exception 'team_not_found';
  end if;
  if p_players is null or jsonb_typeof(p_players) <> 'array' then
    raise exception 'team_size_mismatch';
  end if;
  if jsonb_array_length(p_players) <> v_team_size then
    raise exception 'team_size_mismatch';
  end if;
  if char_length(btrim(coalesce(p_team_name, ''))) > 80 then
    raise exception 'invalid_team_name';
  end if;
  if exists (
    select 1
      from jsonb_to_recordset(p_players)
        as input(id uuid, display_name text, nationality text)
     where char_length(btrim(coalesce(input.display_name, ''))) not between 1 and 60
        or input.nationality is null
        or input.nationality not in ('brit', 'kiwi')
  ) then
    raise exception 'invalid_player_details';
  end if;
  if (
    select count(distinct input.id)
      from jsonb_to_recordset(p_players)
        as input(id uuid, display_name text, nationality text)
  ) <> v_team_size then
    raise exception 'duplicate_player';
  end if;

  select count(*)
    into v_player_count
    from public.player player
    join jsonb_to_recordset(p_players)
      as input(id uuid, display_name text, nationality text)
      on input.id = player.id
   where player.tournament_id = p_tournament_id
     and player.team_id = p_team_id;
  if v_player_count <> v_team_size then
    raise exception 'player_not_in_team';
  end if;

  update public.team
     set name = nullif(btrim(p_team_name), '')
   where id = p_team_id
     and tournament_id = p_tournament_id;

  update public.player as player
     set display_name = btrim(input.display_name),
         nationality = input.nationality::public.nationality
    from jsonb_to_recordset(p_players)
      as input(id uuid, display_name text, nationality text)
   where player.id = input.id
     and player.tournament_id = p_tournament_id
     and player.team_id = p_team_id;
  get diagnostics v_updated = row_count;
  if v_updated <> v_team_size then
    raise exception 'player_update_mismatch';
  end if;

  -- Keep login greetings and the roster display in sync.
  update public.profile as profile
     set display_name = btrim(input.display_name)
    from public.player as player
    join jsonb_to_recordset(p_players)
      as input(id uuid, display_name text, nationality text)
      on input.id = player.id
   where profile.id = player.profile_id
     and player.tournament_id = p_tournament_id
     and player.team_id = p_team_id;
end;
$$;

create or replace function public.delete_setup_team(
  p_tournament_id uuid,
  p_team_id uuid
)
returns table(profile_id uuid)
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_status public.tournament_status;
  v_player_ids uuid[];
  v_profile_ids uuid[];
begin
  select status
    into v_status
    from public.tournament
   where id = p_tournament_id
   for update;

  if not found then
    raise exception 'tournament_not_found';
  end if;
  if v_status <> 'setup' then
    raise exception 'roster_locked';
  end if;
  if not exists (
    select 1
      from public.team
     where id = p_team_id
       and tournament_id = p_tournament_id
  ) then
    raise exception 'team_not_found';
  end if;

  select
    coalesce(array_agg(player.id), '{}'::uuid[]),
    coalesce(
      array_agg(player.profile_id)
        filter (where not profile.is_owner and not profile.is_admin),
      '{}'::uuid[]
    )
    into v_player_ids, v_profile_ids
    from public.player as player
    join public.profile as profile on profile.id = player.profile_id
   where player.tournament_id = p_tournament_id
     and player.team_id = p_team_id;

  -- Generic vote targets have no foreign key, so clear any rehearsal ballots
  -- that refer to the removed team/players or were cast by their old logins.
  delete from public.award_vote
   where tournament_id = p_tournament_id
     and (
       target_id = p_team_id
       or target_id = any(v_player_ids)
       or voter_id = any(v_profile_ids)
     );
  delete from public.player
   where tournament_id = p_tournament_id
     and team_id = p_team_id;
  delete from public.team
   where id = p_team_id
     and tournament_id = p_tournament_id;

  return query select unnest(v_profile_ids);
end;
$$;

revoke all on function public.update_setup_team(uuid, uuid, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.update_setup_team(uuid, uuid, text, jsonb)
  to service_role;

revoke all on function public.delete_setup_team(uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.delete_setup_team(uuid, uuid)
  to service_role;
